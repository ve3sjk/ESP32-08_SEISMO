...
#define PIN_ADC_VBAT          36_uint8

// Digital pin of button
#define PIN_BTN               4_uint8

// Digital pin of buzzer
#define PIN_BUZZER            25_uint8
// #define PIN_BUZZER_POWER      4_uint8

// Digital pin of charger detect signal
// #define PIN_CHARGER_DETECT    16_uint8


// Digital pin of disable charging signal
#define PIN_CHARGING_STOP     18_uint8

// Digital pins of LEDs
#define PIN_LED_CHARGER       32_uint8
#define PIN_LED_LOW_BAT       13_uint8
#define PIN_LED_CALIBRATION   33_uint8
#define PIN_LED_IMU_READING   2_uint8
#define PIN_LED_ALARM         27_uint8

// MPU-6050 I2C address
#define IMU_ADDRESS           0x69_uint8
...
void calibration(void) {
  ...
  if (calibration_state == CALIBRATION_STATE_IN_PROCESS) {
    ...
    if (millis() - calibration_led_timer >= CALIBRATION_LED_BLINK) {
      ...
      if (calibration_led_state == true){
        strip.setLedColorData(0, m_color[1][0], m_color[1][1], m_color[1][2]);
        strip.show();
      }
      if (calibration_led_state == false){
        strip.setLedColorData(0, m_color[8][0], m_color[8][1], m_color[8][2]);
        strip.show();
      }       
    }
  }
  ...
  // Turn LED off if not calibration
  else{
    strip.setLedColorData(0, m_color[8][0], m_color[8][1], m_color[8][2]);
    strip.show();
...
void send_data(void) {
  ...
  tx_buffer[0] = acc_x >> 8;
  tx_buffer[1] = acc_x;
  // Serial.printf("%04x", __builtin_bswap16(acc_x));
  // Serial.print("    ");
...
void buzzer_timer_callback(void) {
  ...
  if (alarm_state == ALARM_STATE_LOW) {
    ...
    if (!alarm_low_state) {
      ...
      analogWrite(PIN_BUZZER,  ALARM_PWM_MAX);
  ...
  // High alarm
    ...
    strip.setLedColorData(0, m_color[5][0], m_color[5][1], m_color[5][2]);
    strip.show();       
  }
...
void imu_setup(void) {
  ...
  Wire.endTransmission();
  Serial.println("imu settup complete");
}
...
void check_charging(void) {
  ...
  charger_connected = 0; // force no charger
}
...
void init_mpu6050(){
  ...
  // pinMode(PIN_BUZZER_POWER, OUTPUT);
  ...
  // timerAlarmWrite(Timer2, ALARM_SOUND_PERIOD, true);
  // timerAlarmEnable(Timer2);

  // Timer1 = timerBegin(1, 80, true);
  // timerAttachInterrupt(Timer1, &onTimer1, true);
  // timerAlarmWrite(Timer1, IMU_READ_PERIOD, true);
  ...
  led_low_bat(false);
  led_alarm(false);
  // digitalWrite(PIN_BUZZER_POWER, HIGH);  
